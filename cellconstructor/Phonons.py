...
#                    raise ValueError("Error, only ibrav 0 supported up to now")
    ...
    def LoadFromPhonopy(self, file_cell, nq=1, full_name = False, allq=True):
        r"""
        This Function loads the phonons information from the quantum espresso dynamical matrix generated by Phonopy
        the file_cell contains information about the structure of the unit cell and the system. An expemple of its structure 
        is provided in the appropriate section.
        Dynamical matrix information are passed by using the qpoints.yaml file produced by Phonopy.
        At the moment the possibility to pass only the irreducible q points is forbidden, you should pass the full q-grid
        by neglecting the mesh symmetry in Phonopy.
        All the dynamical matrices are loaded.
        
        
        Parameters
        ----------
            - file_cell: type(string)
                System and unit cell information
            - nq : type(int), default 1
                Number of total q points in the space group (supercell phonons).
                If 0 or negative an exception is raised.
            - full_name : bool, optional
                If it is True, then the dynamical matrix is loaded without appending the q index.
                This is compatible only with gamma point matrices.
            - all_q : bool
                If true, all the q points in the mesh, neglecting symmetries are provided.
                At the moment, it is the only option possinle.
        """

        if nq <= 0:
            raise ValueError("Error, the specified nq is not valid: it must be positive!")

        if full_name and nq > 1:
            raise ValueError("Error, with full_name only gamma matrices are loaded.")
